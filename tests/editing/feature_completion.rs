...
/// Method chain: `$obj->getUser()->$0` — mir does not record return-type symbols
/// for call expressions, so chained calls currently fall back to the global list.
/// This test documents the limitation so that a future fix can update the snapshot.
#[tokio::test]
async fn completion_method_chain_falls_back_to_global_list() {
    let mut s = TestServer::new().await;
    s.validate_syntax(false);
    let out = s
        .check_completion_ordered(
...
"#,
        ...
        .await;
    // Falls back to the full default list — keywords and builtins appear alongside class symbols.
    expect![[r#"
        Variable    $GLOBALS | superglobal
        Variable    $_COOKIE | superglobal
        Variable    $_ENV | superglobal
        ...
        Function    function_exists
        Function    fwrite
        Method      getUser | function getUser(): User
        Function    get_class
        Function    get_parent_class
        Function    gettype
        Function    glob
        Keyword     global
        Keyword     goto
        Method      greet | function greet(): void
        Function    hash
        Function    header
        Function    headers_sent
        Function    htmlentities
