...
mod tests {
    ...
    fn doc(path: &str, source: &str) -> (Url, Arc<ParsedDoc>) {
        ...
        (url, Arc::new(ParsedDoc::parse(source.to_string())))
    }
    ...
    fn class_const_access_span_covers_only_member_name() {
        // `Status::ACTIVE` — the reference span must cover only `ACTIVE` (starting
        // at character 13), not the whole `Status::ACTIVE` expression (starting at 5).
        // Cannot be verified through the LSP protocol because the server does not
        // yet resolve class-constant references via cursor position.
        let src = "<?php\n$x = Status::ACTIVE;";
    ...
    fn class_const_access_no_duplicate_when_name_equals_class() {
        // `Status::Status` — the walker must emit two distinct refs (class-side at
        // char 5 and member-side at char 13), not two refs both at char 5.
        // Cannot be verified through the LSP protocol for the same reason as above.
        let src = "<?php\n$x = Status::Status;";
        let docs = vec![doc("/a.php", src)];
